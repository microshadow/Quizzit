var _createClass = function () { function defineProperties(target, props) { for (var i = 0; i < props.length; i++) { var descriptor = props[i]; descriptor.enumerable = descriptor.enumerable || false; descriptor.configurable = true; if ("value" in descriptor) descriptor.writable = true; Object.defineProperty(target, descriptor.key, descriptor); } } return function (Constructor, protoProps, staticProps) { if (protoProps) defineProperties(Constructor.prototype, protoProps); if (staticProps) defineProperties(Constructor, staticProps); return Constructor; }; }();

function _classCallCheck(instance, Constructor) { if (!(instance instanceof Constructor)) { throw new TypeError("Cannot call a class as a function"); } }

function _possibleConstructorReturn(self, call) { if (!self) { throw new ReferenceError("this hasn't been initialised - super() hasn't been called"); } return call && (typeof call === "object" || typeof call === "function") ? call : self; }

function _inherits(subClass, superClass) { if (typeof superClass !== "function" && superClass !== null) { throw new TypeError("Super expression must either be null or a function, not " + typeof superClass); } subClass.prototype = Object.create(superClass && superClass.prototype, { constructor: { value: subClass, enumerable: false, writable: true, configurable: true } }); if (superClass) Object.setPrototypeOf ? Object.setPrototypeOf(subClass, superClass) : subClass.__proto__ = superClass; }

import React, { Component } from 'react';

import Notification from './dashboardNotification.js';
import { STUDENT, createHorizontalDivider, createVerticalDivider } from './globals.js';

var EventNotification = function (_Component) {
  _inherits(EventNotification, _Component);

  function EventNotification(props) {
    _classCallCheck(this, EventNotification);

    return _possibleConstructorReturn(this, (EventNotification.__proto__ || Object.getPrototypeOf(EventNotification)).call(this, props));
  }

  _createClass(EventNotification, [{
    key: 'render',
    value: function render() {
      var exclaimedTitle = 'ACTIVE - ' + this.props.title;

      return React.createElement(
        Notification,
        { title: exclaimedTitle,
          href: this.props.href
        },
        React.createElement(
          'div',
          { className: 'notificationExtras' },
          this.props.children
        ),
        React.createElement(
          'div',
          { className: 'notificationDescription' },
          this.props.description
        ),
        createVerticalDivider(12, "background-light"),
        React.createElement(
          'div',
          { className: 'notificationMeta' },
          'Number of Questions: ',
          React.createElement(
            'span',
            { className: 'figure' },
            this.props.meta.numQuestions
          ),
          React.createElement('br', null),
          'Number of Students Writing: ',
          React.createElement(
            'span',
            { className: 'figure' },
            this.props.meta.numStudents
          )
        )
      );
    }
  }]);

  return EventNotification;
}(Component);

export default EventNotification;